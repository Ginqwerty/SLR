...
class DataModule(d2l.HyperParameters):
    ...
    def get_tensorloader(self, tensors, train, indices=slice(0, None)):
        ...
        return torch.utils.data.DataLoader(dataset, self.batch_size,
                                           shuffle=train)
    # get_tensorloader 是将张量数据转换为 DataLoader 的方法。
    # 输入参数：
    # tensors：一个包含张量的元组，每个张量代表数据的一个维度或部分（如输入和标签）。
    # train：布尔值，表示是否为训练集。如果为训练集，会将数据加载器中的 shuffle 参数设为 True，启用数据的随机打乱。
    # indices：用于从 tensors 中选择子集的索引范围。默认选择所有数据。
    # 处理流程：
    # 首先，根据传入的 indices 参数，对每个张量进行切片操作（选择子集），并将切片后的张量组合成一个新的元组。
    # 然后，使用 torch.utils.data.TensorDataset 将张量组合成一个数据集。
    # 最后，通过 torch.utils.data.DataLoader 创建一个数据加载器，将数据集加载为批量张量，批量大小为 self.batch_size。
...
class MTFraEng(d2l.DataModule):
    ...
    def _build_arrays(self, raw_text, src_vocab=None, tgt_vocab=None):
        ...
        def _build_array(sentences, vocab, is_tgt=False):
            #在 _build_array 函数中定义了一个 pad_or_trim 的匿名函数，该函数接收一个句子 seq 和目标长度 t 作为参数。
            #如果句子的长度大于 t，它将截取句子的前 t 个元素。
            #如果句子的长度小于 t，则会在句子后面填充 <pad> 直到句子的长度达到 t
